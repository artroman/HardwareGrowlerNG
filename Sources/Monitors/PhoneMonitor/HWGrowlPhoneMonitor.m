...
-(id)init {
	...
	if((self = [super init])){
		...
		if(serviceDict){
            /*OReturn result = IOBluetoothAddServiceDict((CFDictionaryRef)serviceDict, NULL);
            if(result != kIOReturnSuccess){
                NSLog(@"Error 0x%x", result);
            }*/
            
            // IOBluetoothAddServiceDict registered this Hands-Free SDP record with
            // the classic Bluetooth stack. That API has been removed from the SDK
            // with no replacement, so this monitor can no longer advertise the
            // service — it can still observe HandsFree connections below.
			NSLog(@"HardwareGrowler: skipping HandsFree SDP registration "
					"(IOBluetoothAddServiceDict is no longer available)");
		}else{
			NSLog(@"couldnt read");
		}
	}
